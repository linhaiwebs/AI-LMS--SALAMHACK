...
const MatrailCardItem = ({ item, studyTypeContent }) => {
    if (!item || !studyTypeContent) return null; // Ensure valid props

    const type = item.types; // Correct access to `types`
    ...
    const hasContent = () => {
        ...
        switch (type) {
            ...
            case "qa":
            case "questionanswer":
                return studyTypeContent.qa !== null;
            default:
                return false;
        }
    };
    ...
    return (
        ...
        >
            ...
            <Button  >
                {contentAvailable ? "View" : "Generate"}
            </Button>
        </div>
    );
};

export default MatrailCardItem;
